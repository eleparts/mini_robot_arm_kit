...
* 제작 : eleparts 부설연구소
* SW ver. 1.32
* Servo 모터 4개 + 조이스틱(analog data) + SW 4개 제어 + 보조기능 평행이동 / 기억-실행(속도제어)
**************************************************************/
#include <Servo.h>
...
#define TRUE 1               // 기본 명칭 정의
#define FALSE 0
#define FRONT 1
#define BACK 0

/* 보조기능 2 정의 */
#define SAVE_DATA_MAX_NUMBER 30     // 보조기능 기억-실행 최대 저장횟수
#define FLAG_SAVE 1
#define FLAG_LOAD 0
...
const int SubsysSwA = 3;     // 보조 옵션(평행이동)기능 스위치 K pin 번호  
const int SvaeSwF = 13;       // F 스위치 pin 번호  
const int LoadSwE = 12;      // E 스위치 pin 번호  
...
int swSubA = 1, swSave = 1, swLoad = 1;                       
...
void setup() {  
  ...
  pinMode(SvaeSwF, INPUT_PULLUP);
  pinMode(LoadSwE, INPUT_PULLUP); 
...
void loop() {
  ...
  if(swSubA == FALSE){              // 보조기능 동작유무를 먼저 확인해줌
    parallelMovement();
    delay(500);
  }else if((swSave == FALSE) || (swLoad == FALSE)){
    saveLoadMovement();
    delay(500);
  }
...
void readPin()
{
  ...
  swSave = digitalRead(SvaeSwF);
  swLoad = digitalRead(LoadSwE);
}
...
/* -------- 평행이동의 방향이 이상할경우 위 코드를 아래와 같이 수정 (++ / -- 부분)
...
/* -------- 평행이동의 방향이 이상할경우 위 코드를 아래와 같이 수정 (MIN_ANGLE / MAX_ANGLE 부분)
...
int angleDataBae[SAVE_DATA_MAX_NUMBER][4] = {0,};

int DataSave(int saveCnt){

  if(saveCnt < SAVE_DATA_MAX_NUMBER ){

    angleDataBae[saveCnt][0] = angleHand;
    angleDataBae[saveCnt][1] = angleArmTop;
    angleDataBae[saveCnt][2] = angleArmBot;
    angleDataBae[saveCnt][3] = angleProp;
...
int diffMoterAngle(int readValue, int LimitValue){

  if(readValue > LimitValue){
    return -1;
  }else if(readValue < LimitValue){
    return 1;
  }else{
    return 0;  
  }
...
int slowMove(int moveCnt){

  int handAngle,armTopAngle,armBotAngle,propAngle;

  handAngle = diffMoterAngle(Hand.read(),angleDataBae[moveCnt][0]);
  armTopAngle = diffMoterAngle(ArmTop.read(),angleDataBae[moveCnt][1]);
  armBotAngle = diffMoterAngle(ArmBot.read(),angleDataBae[moveCnt][2]);
  propAngle = diffMoterAngle(Prop.read(),angleDataBae[moveCnt][3]);

  Hand.write(Hand.read() + handAngle);
  ArmTop.write(ArmTop.read() + armTopAngle);
  ArmBot.write(ArmBot.read() + armBotAngle);
  Prop.write(Prop.read() + propAngle);

  return (handAngle || armTopAngle || armBotAngle || propAngle);
}


void loadMovement(int moveDataCnt){

  int moveCnt = 0;
  int loopflag = 1;

  for(moveCnt = 0; moveCnt < moveDataCnt; moveCnt++)
  {
    while(loopflag){
      loopflag = slowMove(moveCnt);
      delay(10);
    }
    loopflag = 1;
  ...
  angleHand = angleDataBae[moveCnt-1][0];
  angleArmTop = angleDataBae[moveCnt-1][1];
  angleArmBot = angleDataBae[moveCnt-1][2];
  angleProp = angleDataBae[moveCnt-1][3];
...
void saveLoadMovement(){
  ...
  static int flag_SL = FLAG_LOAD;                  

  if(swSave == FALSE){
    if(flag_SL == FLAG_LOAD){
      flag_SL = FLAG_SAVE;
      saveCnt = 0;
    }
    saveCnt = DataSave(saveCnt);

  }else if(swLoad == FALSE){
    if(flag_SL == FLAG_SAVE){

      flag_SL = FLAG_LOAD;
    }
    loadMovement(saveCnt);
  }
  delay(300);
}
